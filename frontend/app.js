...
// ── Init ───────────────────────────────────────────────────
function init() {
    // Check for a connect link in the URL hash (#c=ENCODED)
    const hash = window.location.hash;
    if (hash.startsWith('#c=')) {
        handleConnectLink(hash.substring(3));
        return;
    }
...
/**
 * Processes a one-time connect code generated by /webcommands genlink.
 * Decodes base64url(publicApiUrl|token), calls /api/redeem to validate,
 * then saves the API URL and proceeds to login.
 */
async function handleConnectLink(encoded) {
    el('overlay').classList.remove('hidden');
    try {
        const b64     = encoded.replace(/-/g, '+').replace(/_/g, '/');
        const decoded = atob(b64);
        const pipeIdx = decoded.lastIndexOf('|');
        if (pipeIdx < 0) throw new Error('bad format');

        const resolvedApiUrl = decoded.substring(0, pipeIdx);
        const connectToken   = decoded.substring(pipeIdx + 1);

        const res = await fetch(resolvedApiUrl + '/api/redeem', {
            method: 'POST',
        ...
        history.replaceState(null, '', window.location.pathname);
        showSetup();
        showErr(el('setup-error'), '⚠ El código ya fue usado o expiró. Genera uno nuevo con /webcommands genlink en el servidor.');
    } catch {
        el('overlay').classList.add('hidden');
        history.replaceState(null, '', window.location.pathname);
        showSetup();
        showErr(el('setup-error'), '⚠ Código inválido o no se pudo conectar al servidor.');
    }
}
...
function showSetup() {
    ...
    el('overlay').classList.remove('hidden');
    el('input-connect-code').value = '';
}

el('btn-connect').addEventListener('click', async () => {
    const val    = el('input-connect-code').value.trim();
    const errEl  = el('setup-error');
    hideErr(errEl);

    if (!val) {
        showErr(errEl, 'Pega el código generado con /webcommands genlink.');
        return;
    }

    el('modal-setup').classList.add('hidden');
    await handleConnectLink(val);
});

el('input-connect-code').addEventListener('keydown', e => {
